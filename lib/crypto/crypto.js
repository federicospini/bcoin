...
crypto.createMerkleTreePureJS = function createMerkleTreePureJS(leaves) {
  var nodes = leaves;
  ...
  for (j = 0; size > 1; size = ((size + 1) / 2) | 0) {
    for (i = 0; i < size; i += 2) {
      ...
      left = nodes[j + i];
      right = nodes[j + k];
      // console.log(`left: ${j+i}, right: ${j+k}, len: ${leaves.length}, size: ${size}, j: ${j}, i: ${i}, k: ${k}, malleated: ${k === i + 1 && k + 1 === size && left.compare(right) === 0}, k == (i+1): ${k === i + 1}, k+1 == size: ${k+1 === size}, left == right: ${left.compare(right) === 0}`);
...
if (native) {
  ...
  crypto.createMerkleTree = crypto.createMerkleTreePureJS;
}

var merkleTreeShapeCache = []
crypto.merkleTreeShape = function merkleTreeShape (nNodes) {
  var info
  var n

  if (merkleTreeShapeCache[nNodes] !== undefined) {
    return this[nNodes]
  }

  if (merkleTreeShapeCache['n'] > 2) {
    n = merkleTreeShapeCache['n']
  } else {
    merkleTreeShapeCache[0] = []
    merkleTreeShapeCache[1] = [1]
    n = merkleTreeShapeCache['n'] = 2
  }

  while (n <= nNodes) {
    info = _merkleTreeShape(n)
    merkleTreeShapeCache[info.nNodes] = info.shape
    n += 1
  }

  merkleTreeShapeCache['n'] = nNodes
  return merkleTreeShapeCache[nNodes]
}

function _merkleTreeShape (nLeaves) {
  var shape = []
  var nNodes = 0
  while (nLeaves > 2) {
    shape.push(nLeaves)
    nNodes += nLeaves
    nLeaves = ((nLeaves / 2) | 0) + (nLeaves % 2)
    // console.log(`n: ${nNodes}, nLeaves: ${nLeaves}`)
  }
  shape.push(2, 1)
  nNodes += 3
  return { shape, nNodes }
...
var getLeavesNumberCache = []
crypto.getLeavesNumber = function getLeavesNumber (nNodes) {
  var n

  if (getLeavesNumberCache[nNodes] !== undefined) {
    return getLeavesNumberCache[nNodes]
  }

  if (getLeavesNumberCache['n'] > 2) {
    n = getLeavesNumberCache['n']
  } else {
    getLeavesNumberCache[0] = 0
    getLeavesNumberCache[1] = 1
    n = getLeavesNumberCache['n'] = 2
  }

  while (n <= nNodes) {
    getLeavesNumberCache[getNodesNumber(n)] = n
    n += 1
  }

  getLeavesNumberCache['n'] = nNodes
  return getLeavesNumberCache[nNodes]
}

function getNodesNumber (nLeaves) {
  var nNodes = 0
  while (nLeaves > 2) {
    nNodes += nLeaves
    nLeaves = ((nLeaves / 2) | 0) + (nLeaves % 2)
...
function generateMerkleTreeNode (left, right, swap) {
  var lr = new Buffer(64)
  left.copy(lr, (swap ^ 1) * 32)
  right.copy(lr, swap * 32)
  return crypto.hash256(lr)
}
...
crypto.updateMerkleTreePureJS = function updateMerkleTree (tree, leaf, index) {
  // a test could be useful to ensure that
  // the index parameter is less than the number of tree's leaves

  var nodes = tree.nodes
  var nNodes = nodes.length
  var size = crypto.getLeavesNumber(nNodes)
  var newNode = leaf
  var min = Math.min
  var j = 0
  var swap
  var sibling
  var siblingIndex
  var siblingLevelIndex

  for (; size > 1; size = (size + 1) / 2 | 0) {
    nodes[index + j] = newNode
    siblingLevelIndex = min(index ^ 1, size - 1)
    siblingIndex = siblingLevelIndex + j
    // the last bit of the index is 0 if it is on the left,
    // 1 otherwise (i.e. the node is on the right)
    swap = siblingLevelIndex & 1
    sibling = nodes[siblingIndex]

    // console.log(`index: ${index}, j: ${j}, siblingLevelIndex: ${siblingLevelIndex}, siblingIndex: ${siblingIndex}, swap: ${swap}, newNode: ${newNode.toString('hex')}`)

    newNode = generateMerkleTreeNode(newNode, sibling, swap)
    index >>>= 1
    j += size
  }

  // update the root
  nodes[nNodes - 1] = newNode

  return tree
}

crypto.updateMerkleTree = crypto.updateMerkleTreePureJS
