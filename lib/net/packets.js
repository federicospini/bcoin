...
/**
 * Represents a `getblocksmock` packet.
 * @constructor
 * @param {height} height
...
function GetBlocksPacketMock(height) {
  if (!(this instanceof GetBlocksPacketMock))
    return new GetBlocksPacketMock(height);
...
util.inherits(GetBlocksPacketMock, Packet);

GetBlocksPacketMock.prototype.cmd = 'getblocksmock';
GetBlocksPacketMock.prototype.type = exports.types.GETBLOCKSMOCK;
...
GetBlocksPacketMock.prototype.getSize = function getSize() {
  var size = 0;
  size += 4;
  size += 4;
  return size;
...
GetBlocksPacketMock.prototype.toWriter = function toWriter(bw) {
  var i;
...
GetBlocksPacketMock.prototype.toRaw = function toRaw() {
  var size = this.getSize();
  return this.toWriter(new StaticWriter(size)).render();
};
...
GetBlocksPacketMock.prototype.fromReader = function fromReader(br) {
  this.version = br.readU32();
...
GetBlocksPacketMock.prototype.fromRaw = function fromRaw(data) {
  return this.fromReader(new BufferReader(data));
};
...
GetBlocksPacketMock.fromRaw = function fromRaw(data, enc) {
  if (typeof data === 'string')
    data = new Buffer(data, enc);
  return new GetBlocksPacketMock().fromRaw(data);
};
...
function GetHeadersPacket(height) {
  if (!(this instanceof GetHeadersPacket))
    return new GetHeadersPacket(locator, stop);

  GetBlocksPacket.call(this, height);
}

util.inherits(GetHeadersPacket, GetBlocksPacketMock);
