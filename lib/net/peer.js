...
Peer.PING_INTERVAL = 1000 + Math.ceil(Math.random() * 4000);
...
Peer.prototype.sendVersion = function sendVersion() {
  ...
  packet.height = 0; // this.options.getHeight();
...
Peer.prototype.handlePing = co(function* handlePing(packet) {
  ...
  this.send(new packets.PongPacket(packet.nonce));
    ...
    // console.log('PING...')
    // this.emit('ping-received', packet)
});
...
Peer.prototype.handlePong = co(function* handlePong(packet) {
  ...
  this.challenge = null;
    ...
    // console.log('...PONG')
    // this.emit('pong-received', packet)
});
...
Peer.prototype.sendGetHeaders = function sendGetHeaders(locator, stop) {
  ...
  var packet = new packets.GetHeadersPacket(locator);
  var hash = null;
  var end = null;
  ...
  this.logger.debug(
    'Sending getheaders (from=%s).', locator); // locator === height
...
/**
 * Send `getblocksmock` to peer.
 * @param {Hash[]} locator - Chain locator.
 * @param {Hash?} stop - Hash to stop at.
 */

Peer.prototype.sendGetBlocksMock = function getBlocksMock(height) {
  var packet = new packets.GetBlocksPacketMock(height);

  this.logger.debug(
    'Requesting inv packet from peer with getblocksmock (%s).',
    this.hostname());

  this.logger.debug(
    'Sending getblocksmock (height=%s).',
    height);
