...
Pool.prototype.startSyncMock = function startSyncMock({hashes, chain, map}) {
  if (!this.loaded)
    return;
  ...
  this.syncing = true;
  this.syncMock({hashes, chain, map});
};
...
/**
 * Send a sync to each peer MOCK.
 * @private
 */

Pool.prototype.syncMock = function syncMock({hashes, chain, map}) {
  var peer;
  ...
  for (peer = this.peers.head(); peer; peer = peer.next) {
    if (!peer.outbound)
      continue;
    this.sendSyncMock({peer, hashes, chain, map});
  }
};
...
/**
 * Start syncing from peer MOCK.
 * @method
 * @param {Peer} peer
 * @returns {Promise}
 */

Pool.prototype.sendSyncMock = co(function* sendSyncMock({peer, headers, chain, map}) {
  var locator;
  ...
  // in this mock example locator is just the chain height
  locator = chain.length;
  peer.sendGetBlocks(locator);
...
Pool.prototype.handlePacket = co(function* handlePacket(peer, packet) {
  switch (packet.type) {
    ...
    case packetTypes.GETBLOCKSMOCK:
      yield this.handleGetBlocksMock(peer, packet);
      break;
...
Pool.prototype.handlePing = co(function* handlePing(peer, packet) {
    this.logger.info(
        'PING FROM %s, POOL INFO: (hosts=%d, peers=%d)',
        peer.hostname(),
        this.hosts.size(),
        this.peers.size());
});
...
Pool.prototype.handlePong = co(function* handlePong(peer, packet) {
    this.logger.info(
        'PONG FROM %s, POOL INFO: (hosts=%d, peers=%d)',
        peer.hostname(),
        this.hosts.size(),
        this.peers.size());
});
...
Pool.prototype.handleGetData = co(function* handleGetData(peer, packet) {
  ...
  for (i = 0; i < items.length; i++) {
    ...
    if (item.isTX()) {
      ...
      peer.send(new packets.TXPacket(tx, false ));
...
Pool.prototype.handleCmpctBlock = co(function* handleCmpctBlock(peer, packet) {
  var block = packet.block;
  var hash = block.hash('hex');
    var witness = false; // peer.hasWitness();
